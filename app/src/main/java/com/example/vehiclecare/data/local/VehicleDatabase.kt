...
interface VehicleDao {
    @Query("SELECT * FROM vehicles ORDER BY isDefault DESC, brand, model")
    fun observeVehicles(): Flow<List<VehicleEntity>>
    @Query("SELECT * FROM vehicles WHERE id = :id")
    fun observeVehicle(id: Long): Flow<VehicleEntity?>
    @Query("SELECT * FROM maintenance_tasks WHERE vehicleId = :vehicleId ORDER BY isCompleted, dueDate")
    fun observeMaintenance(vehicleId: Long): Flow<List<MaintenanceEntity>>
    @Query("SELECT * FROM documents ORDER BY expiresAt")
    fun observeDocuments(): Flow<List<DocumentEntity>>
    @Query("SELECT * FROM service_points ORDER BY distanceKm")
    ...
    @Query("UPDATE maintenance_tasks SET isCompleted = 1, lastCompletedAt = :completedAt WHERE id = :taskId")
    suspend fun completeTask(taskId: Long, completedAt: LocalDate)
    ...
    @Upsert
    suspend fun upsertMaintenance(items: List<MaintenanceEntity>)
    @Upsert
    suspend fun upsertDocuments(items: List<DocumentEntity>)
    @Upsert
    suspend fun upsertServices(items: List<ServiceEntity>)
}
...
@Database(
    entities = [VehicleEntity::class, MaintenanceEntity::class, DocumentEntity::class, ServiceEntity::class],
    version = 1,
    exportSchema = true
)
