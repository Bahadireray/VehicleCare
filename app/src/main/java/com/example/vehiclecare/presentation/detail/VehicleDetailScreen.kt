...
fun VehicleDetailScreen(
    ...
    if (state.showMileageDialog) MileageDialog(
        onDismiss = { onEvent(VehicleDetailUiEvent.MileageDialogDismissed) },
        onSave = { onEvent(VehicleDetailUiEvent.MileageSaved(it)) })
    Column(Modifier.fillMaxSize()) {
        TopAppBar(
            title = { Text("Vehicle overview") },
            navigationIcon = {
        ...
        when (val vehicle = state.vehicle) {
            ...
            is UiLoadState.Success -> LazyColumn(
                        ...
                        Column(Modifier.padding(20.dp)) {
                            ...
                            Row(verticalAlignment = Alignment.CenterVertically) {
                                Column(Modifier.weight(1f)) {
                                    Text("Current odometer"); Text(
                                    "${vehicle.data.mileageKm.formatKm()} km",
                                    style = MaterialTheme.typography.headlineMedium,
                                    fontWeight = FontWeight.Bold
                                )
                                }
                                IconButton(onClick = { onEvent(VehicleDetailUiEvent.MileageClicked) }) {
                                    Icon(
                ...
                item {
                    Button(
                        onClick = { onEvent(VehicleDetailUiEvent.DefaultClicked) },
                        modifier = Modifier.fillMaxWidth()
                    ) { Text(if (vehicle.data.isDefault) "Default vehicle" else "Make default vehicle") }
                }
                item {
                    Text(
                ...
                items(
                    ...
                    key = { it.id }) { task ->
                    MaintenanceRow(task) {
                        if (!task.isCompleted) onEvent(
                            VehicleDetailUiEvent.MaintenanceCompleted(task.id)
                        )
                    }
                }
            }
...
@Composable
private fun MileageDialog(onDismiss: () -> Unit, onSave: (String) -> Unit) {
