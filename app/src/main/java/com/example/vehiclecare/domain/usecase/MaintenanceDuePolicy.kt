...
class MaintenanceDuePolicy {
    fun evaluate(task: MaintenanceTask, currentMileageKm: Int, today: LocalDate = LocalDate.now()): MaintenanceDueStatus {
        ...
        return when { dateDue -> MaintenanceDueStatus(true, DueReason.DATE); mileageDue -> MaintenanceDueStatus(true, DueReason.MILEAGE); else -> MaintenanceDueStatus(false, null) }
    }
}
